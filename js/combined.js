...
}(window.Raphael);Raphael.fromJquery = function ($obj) {
  var obj = $obj.get(0),
...
var process = function (json) {
    var x = 0,
        r = Raphael("workflowchart", 1550, 250),
        labels = {},
        ...
        for (var i in pathes) {
            ...
            (function (i) {
                pathes[i].p.mouseover(function () {
                    ...
                    pathes[i].p.toFront();
                    labels[i].toFront();
                    usrnm2.innerHTML = json.authors[i].n + " <em>(" + json.authors[i].c + " tickets during last 120 days)</em>";
...
//var dots_process = function (json) {
	...
	init : function (options) {
		...
		var axisy = options.data.ytitles;
	...
	update: function() {
	  return $(this).each(function() {
		...
		var width = dgs.width,
		        ...
		        color = dgs.color,
		        max = dgs.max;
	    var dgd = $(this).data("dotsgraph_data");
            var xaxis = dgd.xtitles,
		yaxis = dgd.ytitles,
		data = dgd.date_stats;
	    var o = 0;
	    // i == y axis index
	    ...
	    // R == circle diameter
	    // 
            var circleinAnimation = Raphael.animation({opacity: 1}, 750, "easeIn");
            var circleoutAnimation = Raphael.animation({opacity: 1}, 1500, "easeOut");
	    for (var i = 0, ii = dgd.ytitles.length; i < ii; i++) {
	        for (var j = 0, jj = dgd.xtitles.length; j < jj; j++) {
		    var Rtmp = max;
	            var R = data[o] && Math.min(Math.round(Math.sqrt(data[o] / Math.PI / 13) * 4), max);
                    var dttmp = r.circle(leftgutter + X * (j + .5) - 60 - R + 60 + R, Y * (i + .5) - 10 + 10, max+2).attr({stroke:"none", fill: "#fff", opacity: 0});
		    dttmp.animate(circleoutAnimation.delay(dgd.ytitles.length*dgd.xtitles.length - o * 10)); //{"opacity": 1}, 1500, "easeOut");
	            if (R) {
	                (function (dx, dy, R, value) {
	                    ...
	                    var dt = r.circle(dx + 60 + R, dy + 10, R).attr({stroke: "none", fill: color, opacity: 0});
			    dt.animate(circleinAnimation.delay((o % 24)*70 + (o % 24) * 20));
	                    ...
	                    var dot = r.circle(dx + 60 + R, dy + 10, max).attr({stroke: "none", fill: "#00f", opacity: 0});
