 ...
 * Licensed under the MIT (http://www.opensource.org/licenses/mit-license.php) license.
 */
(function(){var i=Math.min,h=Math.max;function j(c,e,v,b,t,r,f,u){var x,d={round:"round",sharp:"sharp",soft:"soft",square:"square"};if((t&&!b)||(!t&&!v)){return f?"":u.path()}r=d[r]||"square";b=Math.round(b);v=Math.round(v);c=Math.round(c);e=Math.round(e);switch(r){case"round":if(!t){var w=~~(b/2);if(v<w){w=v;x=["M",c+0.5,e+0.5-~~(b/2),"l",0,0,"a",w,~~(b/2),0,0,1,0,b,"l",0,0,"z"]}else{x=["M",c+0.5,e+0.5-w,"l",v-w,0,"a",w,w,0,1,1,0,b,"l",w-v,0,"z"]}}else{w=~~(v/2);if(b<w){w=b;x=["M",c-~~(v/2),e,"l",0,0,"a",~~(v/2),w,0,0,1,v,0,"l",0,0,"z"]}else{x=["M",c-w,e,"l",0,w-b,"a",w,w,0,1,1,v,0,"l",0,b-w,"z"]}}break;case"sharp":if(!t){var a=~~(b/2);x=["M",c,e+a,"l",0,-b,h(v-a,0),0,i(a,v),a,-i(a,v),a+(a*2<b),"z"]}else{a=~~(v/2);x=["M",c+a,e,"l",-v,0,0,-h(b-a,0),a,-i(a,b),a,i(a,b),a,"z"]}break;case"square":if(!t){x=["M",c,e+~~(b/2),"l",0,-b,v,0,0,b,"z"]}else{x=["M",c+~~(v/2),e,"l",1-v,0,0,-b,v-1,0,"z"]}break;case"soft":if(!t){w=i(v,Math.round(b/5));x=["M",c+0.5,e+0.5-~~(b/2),"l",v-w,0,"a",w,w,0,0,1,w,w,"l",0,b-w*2,"a",w,w,0,0,1,-w,w,"l",w-v,0,"z"]}else{w=i(Math.round(v/5),b);x=["M",c-~~(v/2),e,"l",0,w-b,"a",w,w,0,0,1,w,-w,"l",v-2*w,0,"a",w,w,0,0,1,w,w,"l",0,b-w,"z"]}}if(f){return x.join(",")}else{return u.path(x)}}function k(ax,ab,ad,az,aw,b,ah){ah=ah||{};var al=this,a=ah.type||"square",ao=parseFloat(ah.gutter||"20%"),d=ax.set(),ag=ax.set(),av=ax.set(),aj=ax.set(),af=Math.max.apply(Math,b),c=[],ac=0,Y=ah.colors||al.colors,ak=b.length;if(Raphael.is(b[0],"array")){af=[];ac=ak;ak=0;for(var f=b.length;f--;){ag.push(ax.set());af.push(Math.max.apply(Math,b[f]));ak=Math.max(ak,b[f].length)}if(ah.stacked){for(var f=ak;f--;){var aq=0;for(var s=b.length;s--;){aq+=+b[s][f]||0}c.push(aq)}}for(var f=b.length;f--;){if(b[f].length<ak){for(var s=ak;s--;){b[f].push(0)}}}af=Math.max.apply(Math,ah.stacked?c:af)}af=(ah.to)||af;var aa=az/(ak*(100+ao)+ao)*100,ay=aa*ao/100,at=ah.vgutter==null?20:ah.vgutter,ai=[],ar=ab+ay,au=(aw-2*at)/af;if(!ah.stretch){ay=Math.round(ay);aa=Math.floor(aa)}!ah.stacked&&(aa/=ac||1);for(var f=0;f<ak;f++){ai=[];for(var s=0;s<(ac||1);s++){var e=Math.round((ac?b[s][f]:b[f])*au),ap=ad+aw-at-e,y=j(Math.round(ar+aa/2),ap+e,aa,e,true,a,null,ax).attr({stroke:"none",fill:Y[ac?s:f]});if(ac){ag[s].push(y)}else{ag.push(y)}y.y=ap;y.x=Math.round(ar+aa/2);y.w=aa;y.h=e;y.value=ac?b[s][f]:b[f];if(!ah.stacked){ar+=aa}else{ai.push(y)}}if(ah.stacked){var x;aj.push(x=ax.rect(ai[0].x-ai[0].w/2,ad,aa,aw).attr(al.shim));x.bars=ax.set();var an=0;for(var Z=ai.length;Z--;){ai[Z].toFront()}for(var Z=0,am=ai.length;Z<am;Z++){var y=ai[Z],ae,e=(an+y.value)*au,X=j(y.x,ad+aw-at-!!an*0.5,aa,e,true,a,1,ax);x.bars.push(y);an&&y.attr({path:X});y.h=e;y.y=ad+aw-at-!!an*0.5-e;av.push(ae=ax.rect(y.x-y.w/2,y.y,aa,y.value*au).attr(al.shim));ae.bar=y;ae.value=y.value;an+=y.value}ar+=aa}ar+=ay}aj.toFront();ar=ab+ay;if(!ah.stacked){for(var f=0;f<ak;f++){for(var s=0;s<(ac||1);s++){var ae;av.push(ae=ax.rect(Math.round(ar),ad+at,aa,aw-at).attr(al.shim));ae.bar=ac?ag[s][f]:ag[f];ae.value=ae.bar.value;ar+=aa}ar+=ay}}d.label=function(r,n){r=r||[];this.labels=ax.set();var m,q=-Infinity;if(ah.stacked){for(var t=0;t<ak;t++){var p=0;for(var v=0;v<(ac||1);v++){p+=ac?b[v][t]:b[t];if(v==ac-1){var u=ax.labelise(r[t],p,af);m=ax.text(ag[t*(ac||1)+v].x,ad+aw-at/2,u).attr(txtattr).insertBefore(av[t*(ac||1)+v]);var o=m.getBBox();if(o.x-7<q){m.remove()}else{this.labels.push(m);q=o.x+o.width}}}}}else{for(var t=0;t<ak;t++){for(var v=0;v<(ac||1);v++){var u=ax.labelise(ac?r[v]&&r[v][t]:r[t],ac?b[v][t]:b[t],af);m=ax.text(ag[t*(ac||1)+v].x,n?ad+aw-at/2:ag[t*(ac||1)+v].y-10,u).attr(txtattr).insertBefore(av[t*(ac||1)+v]);var o=m.getBBox();if(o.x-7<q){m.remove()}else{this.labels.push(m);q=o.x+o.width}}}}return this};d.hover=function(m,n){aj.hide();av.show();av.mouseover(m).mouseout(n);return this};d.hoverColumn=function(m,n){av.hide();aj.show();n=n||function(){};aj.mouseover(m).mouseout(n);return this};d.click=function(m){aj.hide();av.show();av.click(m);return this};d.each=function(m){if(!Raphael.is(m,"function")){return this}for(var n=av.length;n--;){m.call(av[n])}return this};d.eachColumn=function(m){if(!Raphael.is(m,"function")){return this}for(var n=aj.length;n--;){m.call(aj[n])}return this};d.clickColumn=function(m){av.hide();aj.show();aj.click(m);return this};d.push(ag,av,aj);d.bars=ag;d.covers=av;return d}function g(Z,ab,ad,X,ac,aq,aj){aj=aj||{};var at=this,ao=aj.type||"square",an=parseFloat(aj.gutter||"20%"),ag=Z.set(),Y=Z.set(),ak=Z.set(),y=Z.set(),a=Math.max.apply(Math,aq),au=[],W=0,af=aj.colors||at.colors,f=aq.length;if(Raphael.is(aq[0],"array")){a=[];W=f;f=0;for(var s=aq.length;s--;){Y.push(Z.set());a.push(Math.max.apply(Math,aq[s]));f=Math.max(f,aq[s].length)}if(aj.stacked){for(var s=f;s--;){var V=0;for(var x=aq.length;x--;){V+=+aq[x][s]||0}au.push(V)}}for(var s=aq.length;s--;){if(aq[s].length<f){for(var x=f;x--;){aq[s].push(0)}}}a=Math.max.apply(Math,aj.stacked?au:a)}a=(aj.to)||a;var d=Math.floor(ac/(f*(100+an)+an)*100),ai=Math.floor(d*an/100),am=[],ar=ad+ai,ap=(X-1)/a;!aj.stacked&&(d/=W||1);for(var s=0;s<f;s++){am=[];for(var x=0;x<(W||1);x++){var b=W?aq[x][s]:aq[s],e=j(ab,ar+d/2,Math.round(b*ap),d-1,false,ao,null,Z).attr({stroke:"none",fill:af[W?x:s]});if(W){Y[x].push(e)}else{Y.push(e)}e.x=ab+Math.round(b*ap);e.y=ar+d/2;e.w=Math.round(b*ap);e.h=d;e.value=+b;if(!aj.stacked){ar+=d}else{am.push(e)}}if(aj.stacked){var al=Z.rect(ab,am[0].y-am[0].h/2,X,d).attr(at.shim);y.push(al);al.bars=Z.set();var ae=0;for(var ah=am.length;ah--;){am[ah].toFront()}for(var ah=0,U=am.length;ah<U;ah++){var e=am[ah],c,b=Math.round((ae+e.value)*ap),aa=j(ab,e.y,b,d-1,false,ao,1,Z);al.bars.push(e);ae&&e.attr({path:aa});e.w=b;e.x=ab+b;ak.push(c=Z.rect(ab+ae*ap,e.y-e.h/2,e.value*ap,d).attr(at.shim));c.bar=e;ae+=e.value}ar+=d}ar+=ai}y.toFront();ar=ad+ai;if(!aj.stacked){for(var s=0;s<f;s++){for(var x=0;x<(W||1);x++){var c=Z.rect(ab,ar,X,d).attr(at.shim);ak.push(c);c.bar=W?Y[x][s]:Y[s];c.value=c.bar.value;ar+=d}ar+=ai}}ag.label=function(r,n){r=r||[];this.labels=Z.set();for(var o=0;o<f;o++){for(var p=0;p<W;p++){var u=Z.labelise(W?r[p]&&r[p][o]:r[o],W?aq[p][o]:aq[o],a),t=n?Y[o*(W||1)+p].x-d/2+3:ab+5,m=n?"end":"start",q;this.labels.push(q=Z.text(t,Y[o*(W||1)+p].y,u).attr(txtattr).attr({"text-anchor":m}).insertBefore(ak[0]));if(q.getBBox().x<ab+5){q.attr({x:ab+5,"text-anchor":"start"})}else{Y[o*(W||1)+p].label=q}}}return this};ag.hover=function(m,n){y.hide();ak.show();n=n||function(){};ak.mouseover(m).mouseout(n);return this};ag.hoverColumn=function(m,n){ak.hide();y.show();n=n||function(){};y.mouseover(m).mouseout(n);return this};ag.each=function(n){if(!Raphael.is(n,"function")){return this}for(var m=ak.length;m--;){n.call(ak[m])}return this};ag.eachColumn=function(n){if(!Raphael.is(n,"function")){return this}for(var m=y.length;m--;){n.call(y[m])}return this};ag.click=function(m){y.hide();ak.show();ak.click(m);return this};ag.clickColumn=function(m){ak.hide();y.show();y.click(m);return this};ag.push(Y,ak,y);ag.bars=Y;ag.covers=ak;return ag}var l=function(){};l.prototype=Raphael.g;g.prototype=k.prototype=new l;Raphael.fn.hbarchart=function(e,a,c,f,d,b){return new g(this,e,a,c,f,d,b)};Raphael.fn.barchart=function(e,a,c,f,d,b){return new k(this,e,a,c,f,d,b)}})();